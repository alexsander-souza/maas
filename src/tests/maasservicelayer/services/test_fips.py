...
class TestFIPSService:
    ...
    @pytest.mark.asyncio
    async def test_emit_startup_log_on_detection_error_logs_warning(
        self, fips_service
    ):
        """emit_startup_log logs at WARNING when get_fips_status raises."""
        import structlog.testing
        ...
        with (
            patch.object(
                fips_service,
                "get_fips_status",
                side_effect=OSError("cannot read proc file"),
            ),
            structlog.testing.capture_logs() as log_events,
        ):
            await fips_service.emit_startup_log()
        ...
        assert any(
            ...
            f"Expected WARNING-level FIPS_MODE_DETECTED event not found in: {log_events}"
        )
