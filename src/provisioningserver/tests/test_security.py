...
class TestFIPSKeyValidation(MAASTestCase):
    def test_ignores_validation_when_fips_is_disabled(self):
        self.patch(security, "is_fips_enabled").return_value = False

        key = rsa.generate_private_key(public_exponent=65537, key_size=1024)

        security.validate_key_fips_compliance(key)

    def test_rejects_short_rsa_key_when_fips_is_enabled(self):
        self.patch(security, "is_fips_enabled").return_value = True
        mock_logger = self.patch(security, "logger")
        key = rsa.generate_private_key(public_exponent=65537, key_size=1024)

        error = self.assertRaises(
            security.FIPSCryptoError,
            security.validate_key_fips_compliance,
            key,
        )

        self.assertEqual(
            str(error),
            "RSA key size below FIPS minimum of 2048 bits",
        )
    ...
    def test_rejects_dsa_key_when_fips_is_enabled(self):
        self.patch(security, "is_fips_enabled").return_value = True
    ...
    def test_rejects_short_openssl_rsa_key_when_fips_is_enabled(self):
        self.patch(security, "is_fips_enabled").return_value = True
        key = crypto.PKey()
